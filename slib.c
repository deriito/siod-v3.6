...
char *__stdcall siod_version(void) { return ("3.6.2 12-MAY-07"); }
...
void gput_st(struct gen_printio *f, char *st) { PUTS_FCN(st, f); }
...
LISP consp(LISP x) { if CONSP(x) return (sym_t); else return (NIL); }
...
LISP numberp(LISP x) { if FLONUMP(x) return (sym_t); else return (NIL); }
...
LISP eq(LISP x, LISP y) { if EQ(x, y) return (sym_t); else return (NIL); }

LISP eql(LISP x, LISP y) {
    if EQ(x, y) return (sym_t);
    else if NFLONUMP(x) return (NIL);
    else if NFLONUMP(y) return (NIL); else if (FLONM(x) == FLONM(y)) return (sym_t);
    return (NIL);
}
...
LISP symbolp(LISP x) { if SYMBOLP(x) return (sym_t); else return (NIL); }

LISP err_ubv(LISP v) { return (err("unbound variable", v)); }
...
void init_subr_2n(char *name, LISP (*fcn)(LISP, LISP)) { init_subr(name, tc_subr_2n, (SUBR_FUNC) fcn); }
...
void init_subr_5(char *name, LISP (*fcn)(LISP, LISP, LISP, LISP, LISP)) { init_subr(name, tc_subr_5, (SUBR_FUNC) fcn); }
...
LISP err_closure_code(LISP tmp) { return (err("closure code type not valid", tmp)); }
...
LISP fopen_c(char *name, char *how) { return (fopen_cg(fopen, name, how)); }
...
LISP quit(void) { return (err(NULL, NIL)); }

LISP nullp(LISP x) { if EQ(x, NIL) return (sym_t); else return (NIL); }
...
int __stdcall siod_verbose_check(int level) { return ((siod_verbose_level >= level) ? 1 : 0); }
...
LISP lrealtime(void) { return (flocons(myrealtime())); }

LISP caar(LISP x) { return (car(car(x))); }

LISP cadr(LISP x) { return (car(cdr(x))); }

LISP cdar(LISP x) { return (cdr(car(x))); }
...
LISP a_true_value(void) { return (sym_t); }
...
LISP lllast_c_errmsg(void) { return (llast_c_errmsg(-1)); }
...
static LISP parser_read(LISP ignore) { return (leval(cintern("read"), NIL)); }
